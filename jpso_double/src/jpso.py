...
class Particle:
    def __init__(self,network):
        ...
        self.b_objecive = self.c_objecive
        self.b_err = None
    ...
    def update(self,zmax,zmin,c_flag,b_flag):
        c_fv = self.__fv(self.c_objecive,zmax,zmin,c_flag)
        b_fv = self.__fv(self.c_objecive,zmax,zmin,b_flag)
    ...
    1
    # fitness evaluation
    def __fv(self,objecive,zmax,zmin,flag):
        # add 1e-6 in numerator and denominator to handle
        # zero case
        weighted_sum = sum([(objecive[j] - zmin[j] + 1e-6)/(zmax[j]-zmin[j] + 1e-6) for j in range(len(objecive))])
...
class Swarm:
    def __init__(self,network,swarm_size=50,generations=150,stall_gen=20,delta=0.01,C0=0,C1=0.5):
        ...
        for particle in self.swarm:
            # always update zmin,zmax with current objecive of particle
            self.update_objecive(particle.c_objecive)
        
        for particle in self.swarm:
            # update b_err in each particle
            c_flag = self.is_dominated(particle.c_objecive)
            b_flag = self.is_dominated(particle.b_objecive)
    ...
    def is_dominated(self,objecive):
        flag = False

        for particle in self.swarm:
            flag = reduce((lambda x,y:x&y),[x>y for (x,y) in zip(objecive,particle.b_objecive)])
            if flag:
                return flag

        return flag
    
    def update_objecive(self,objecive):
        self.zmax[:] = [max(x,y) for (x,y) in zip(self.zmax,objecive)]
        self.zmin[:] = [min(x,y) for (x,y) in zip(self.zmin,objecive)]
