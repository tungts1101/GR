...
\caption{Mutation}
\begin{algorithmic}[1]
\State Selecting randomly 2 nodes in tree with its distance is smaller than transmission range
\State Create an edge between 2 nodes
\State Remove edge from descendant with its older parent
\end{algorithmic}
\end{algorithm}
...
Both above schemes can lead to an unfeasible solution when leaves are not all Steiner nodes, therefore we utilize the repair routine [\cite]. The repair scheme is visiting all leaves in the tree and cutting off any leaf which is not a Steiner node. And because after node’s children were cut off tree, it can become unacceptable, leaves trimming strategy is applied until all leaves are satisfactory.
